...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        auto yield1 = [this]{ return concurRace1->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value()); };
        auto yield2 = [this]{ return concurRace2->DoWork(&number, false, ui->sb_initNum->value()); };

        QFuture result = QtConcurrent::run(yield1).then(yield2);
    }
    else{
